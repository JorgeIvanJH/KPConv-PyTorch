...
#      Jorge Jaramillo - 2025 (adapted for MiniMarket)
...
class MiniMarketDataset(PointCloudDataset):
    def __init__(self, config, set='training', use_potentials=True):
        ...
        PointCloudDataset.__init__(self,'MiniMarket')
        ############
        # Parameters
        ############

        # Dict from labels to names
        self.label_to_names = {0: 'background_n_clutter',
                               1: 'object'}
        ...
        # Dataset folder
        self.path = r"D:\Datasets\MinimarketPointCloud\MiniMarket_point_clouds\64\segmentation_dataset\ketchup_heinz_400ml_segmentation_date_20250526_time_160358_numPoints_64_maxObjects_10_numOrientations_1.h5"
        ...
        self.dataset_dir = os.path.dirname(self.path)
        ply_path = join(self.dataset_dir, self.train_path)
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for MiniMarket data: ', self.set)

        ###################
        # Prepare ply files
        ###################

        self.prepare_MiniMarket_ply()
        ...
        self.files = sorted([
            join(ply_path, f)
            for f in listdir(ply_path)
            if f.endswith('.ply')
        ])
        split_ratio = 0.8  # for example, 80% training, 20% validation
        ...
        self.cloud_names = [splitext(f)[0] for f in self.files]
    ...
    def prepare_MiniMarket_ply(self, output_folder=None):
        """
        Extracts and converts synthetic point clouds stored in a single HDF5 file into individual PLY files.

        This method:
        1. Reads all 1200 point clouds from the HDF5 file at `self.path`.
        2. Converts each one-hot label to a single integer class.
        3. Saves each cloud as a PLY file with fields ['x', 'y', 'z', 'red', 'green', 'blue', 'class'].

        The PLY files are saved to:
            {output_folder}/cloud_XXXX.ply

        Args:
            output_folder (str, optional): Directory where the PLY files will be saved. If None, uses a subdirectory
                                        named 'self.train_path' in the same location as the HDF5 file.

        Raises:
            ValueError: If input label shape is not (B, N, 2) or if conversion fails.
        """
        print('\nPreparing MiniMarket PLY files...')
        t0 = time.time()

        # Default output path
        if output_folder is None:
            output_folder = os.path.join(self.dataset_dir, self.train_path)
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)

        # Load data from .h5 file
        with h5py.File(self.path, 'r') as f:
            seg_points = f['seg_points'][:]     # (1200, 640, 3)
            seg_colors = f['seg_colors'][:]     # (1200, 640, 3)
            seg_labels = f['seg_labels'][:]     # (1200, 640, 2)

        # Sanity check
        B, N, D = seg_points.shape
        assert seg_labels.shape[-1] == 2, "Labels should be one-hot encoded with shape (B, N, 2)"

        for i in range(B):
            points = seg_points[i]              # (640, 3)
            colors = seg_colors[i]              # (640, 3)
            labels_one_hot = seg_labels[i]      # (640, 2)

            # Convert one-hot to class index (0 or 1)
            labels = np.argmin(labels_one_hot, axis=1).astype(np.int32)  # (640,)

            # Scale color to [0,255] if needed (assuming already uint8 if not float)
            if colors.dtype != np.uint8:
                colors = (colors * 255).astype(np.uint8)

            # Save as .ply
            cloud_filename = os.path.join(output_folder, f'cloud_{i:04d}.ply')
            write_ply(cloud_filename,
                    [points, colors, labels.reshape(-1, 1)],
                    ['x', 'y', 'z', 'red', 'green', 'blue', 'class'])

        print(f'Done. Exported {B} clouds in {time.time() - t0:.1f}s to: {output_folder}')
        # Store cloud names
        self.cloud_names = [f'cloud_{i:04d}' for i in range(B)]
        self.all_splits = list(range(B))
        self.validation_split = int(0.2 * B)  # Or pick an index, e.g., 200
    ...
    def load_subsampled_clouds(self):
        ...
        tree_path = join(self.dataset_dir, 'input_{:.3f}'.format(dl))
...
class MiniMarketSampler(Sampler):
    ...
    def __init__(self, dataset: MiniMarketDataset):
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        batch_lim_file = join(self.dataset.dataset_dir, 'batch_limits.pkl')
        ...
        neighb_lim_file = join(self.dataset.dataset_dir, 'neighbors_limits.pkl')
